...
/**
 * Converts a two or three digit ISO-639 code into a human-readable name that can be used as a Solr facet.
 * <p>
 * ISO 639-1 and ISO 639-2 language codes are taken from the <a
 * href="http://www.loc.gov/standards/iso639-2/ISO-639-2_utf-8.txt">Library of Congress' list</a>.
 * </p>
 *
...
public class ISO639Converter {
    ...
    /**
     * The source file for the ISO-639 mapping (downloaded from the Library of Congress).
     */
     ...
     */
    static {
        final InputStream in = ISO639Converter.class.getResourceAsStream(MAP_FILE);
        final BufferedReader reader = new BufferedReader(new InputStreamReader(in));
        final Map<String, String> iso639_1_map = new HashMap<String, String>();
        final Map<String, String> iso639_2_map = new HashMap<String, String>();
        String line;

        try {
            while ((line = reader.readLine()) != null) {
                final String[] parts = line.split("\\|");
                final String langName = parts[3].split(";")[0];

                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Loading ISO 639 entry: '{}' ({}) = {} {} {}", line, parts.length, parts[0],
                            parts[2], langName);
                }

                // Parsing out primary three digit codes
                if (parts[0].length() > 0 && langName.length() > 0 && iso639_2_map.put(parts[0], langName) != null) {
                    throw new RuntimeException("Source file is corrupt; " + "duplicate map entry for: " + parts[0]);
                }

                // Parsing out secondary three digit codes
                if (parts[1].length() > 0 && langName.length() > 0 && iso639_2_map.put(parts[1], langName) != null) {
                    throw new RuntimeException("Source file is corrupt; " + "duplicate map entry for: " + parts[1]);
                }

                // Parsing out two digit codes
                if (parts[2].length() > 0 && langName.length() > 0 && iso639_1_map.put(parts[2], langName) != null) {
                    throw new RuntimeException("Source file is corrupt; " + "duplicate map entry for: " + parts[2]);
                }

                if (langName.length() <= 0) {
                    throw new RuntimeException("Source file is corrupt; " + "missing a language name for " + parts[0]);
                }
            }
     ...
     */
    public static String convert(final String aCode) {
        String langName;
     ...
     * <p>
     * We don't use an XML processing library here (like XOM), so as not to add a dependency, but the unit test for
     * this class checks to confirm that the XML created here is well-formed.
     * </p>
     *
    ...
    public String toString() {
        final StringBuilder builder = new StringBuilder("<converter><iso-639-1>");
        Iterator<String> iterator = ISO639_1_MAP.keySet().iterator();

        while (iterator.hasNext()) {
            final String key = iterator.next();
            final String value = ISO639_1_MAP.get(key);
        ...
        while (iterator.hasNext()) {
            final String key = iterator.next();
            final String value = ISO639_2_MAP.get(key);
